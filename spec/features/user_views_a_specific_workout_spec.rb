...
RSpec.feature "User views a specific workout" do
  ...
  scenario "workout clicked on from workout listing", js: true do
    user = User.create(user_params)
    map_my_fitness_id = "542541966"
    workout = create(:workout, user: user, map_my_fitness_id: map_my_fitness_id)
    ...
    find("#workout-#{map_my_fitness_id}").click
    ...
    expect(page).to have_content("5278.22 ft")
    expect(page).to have_content("5296.0 ft")
  end
